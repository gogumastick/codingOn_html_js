...
// join(경로들) : 여러개의 경로 조작을 결합해 하나의 경로로 생성
// 상대, 절대, 혼합 경로로 설정될 수 있음
// 상대경로 예시
const pathJoin1 = path.join(__dirname,'public','txt', 'file.txt'); // __dirname은 경로를 보여줌
...
// resolve(경로들) : 여러개의 경로조각을 결합해 절대경로로 생성(경로가 없으면 현재 경로)
...
// 확장자를 입력하지 않으면 전체파일명,
// 확장자를 입력하면 확장자를 제거한 파일명
const filename = path.basename('/users/martin/files/file.txt');
console.log(filename);
const originalName = path.basename('/users/martin/files/file.txt','.txt');
console.log(originalName);

// extname(경로) : 주어진 경로의 확장자를 반환
const ext = path.extname('/users/martin/files/file.txt');
console.log(ext);
// const originalName = path.basename('/users/martin/files/file.txt',ext);
